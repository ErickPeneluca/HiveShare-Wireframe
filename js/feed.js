...
function initializeFeed() {
    ...
    if (currentUser) {
        ...
        document.querySelector('.user-menu').innerHTML = `
            <button class="btn btn-primary" onclick="window.location.href='/HiveShare-Wireframe/index.html'">Entrar</button>
        `;
    ...
    posts = [
        {
            id: Date.now() + 1,
            author: users[1],
            content: "Acabei de finalizar um novo projeto de UI/UX! Quem mais está trabalhando em projetos pessoais hoje? 🎨✨",
        ...
        },
        {
            id: Date.now() + 2,
            author: users[2],
            content: "Descobri uma biblioteca incrível para animações em React! Alguém já usou Framer Motion? Vale muito a pena conferir.",
        ...
        },
        {
            id: Date.now() + 3,
            author: users[1],
            content: "Dica do dia: sempre valide suas ideias com usuários reais antes de investir muito tempo no desenvolvimento. Feedback cedo = menos retrabalho! 💡",
        ...
        },
        {
            id: Date.now() + 4,
            author: users[2],
            content: "Estudando IA generativa e as possibilidades são infinitas! Quem mais está explorando esse universo?",
...
function loadFeed() {
    ...
    if (paginatedPosts.length === 0) {
        if (currentPage === 1) {
            container.innerHTML = '<div class="empty-state">Nenhum post para exibir</div>';
        }
        hasMorePosts = false;
...
function renderPost(post) {
    ...
    return `
        <div class="post-card" data-post-id="${post.id}">
            ...
            ${post.video ? `<video src="${post.video}" controls class="post-image"></video>` : ''}
            
            ${post.tags && post.tags.length > 0 ? `
                <div class="post-tags">
                    ${post.tags.map(tag => `<a href="#" class="post-tag" onclick="filterByTag('${tag}'); return false;">#${tag}</a>`).join('')}
                </div>
            ` : ''}
            ...
            <div class="post-actions">
                ...
                <button class="post-action ${isSaved ? 'saved' : ''}" onclick="toggleSave(${post.id})">
                    ${isSaved ? '🔖' : '📋'}
                </button>
            </div>
        </div>
...
function openCreatePostModal() {
    if (!currentUser) {
        ...
        setTimeout(() => window.location.href = '/HiveShare-Wireframe/index.html', 1500);
        return;
    }
...
function handleCreatePost(event) {
    ...
    if (!content && !currentMediaData) {
        showNotification('Escreva algo ou adicione uma mídia antes de publicar!', 'error');
        return;
    }
...
function renderComments(post) {
    ...
    if (!post.commentsList || post.commentsList.length === 0) {
        commentsList.innerHTML = '<p style="text-align: center; color: var(--text-tertiary); padding: 20px;">Seja o primeiro a comentar!</p>';
        return;
    }
            ...
            <div class="comment-item">
                ...
                <div class="comment-content">
                    ...
                    <div class="comment-actions-bar">
                        <button class="comment-action ${isLiked ? 'liked' : ''}" onclick="toggleCommentLike(${comment.id})">
...
function showPostMenu(postId) {
    ...
    if (currentUser && post.author.id === currentUser.id) {
        if (confirm('Deseja excluir este post? Esta ação não pode ser desfeita.')) {
            deletePost(postId);
        }
    } else {
...
function followUser(userId) {
    ...
    // Salvar no localStorage
    localStorage.setItem(`following_${currentUser.id}`, JSON.stringify(followingUsers));
}
...
function logout() {
    ...
    window.location.href = '/HiveShare-Wireframe/index.html';
}
